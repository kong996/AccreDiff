...
// =============================================================================
// Device function: 径向变化的面密度梯度指数
//   α(r) = G_alpha * ln(r) ; G_alpha==1(default)

__device__ double alpha_r(double r1){
    double a = Gas_parameters_c[1];

    return a * log(r1);
}

// 真实面密度梯度
// -d(lnΣ)/d(lnr) = 2*G_alpha*ln(r)
__device__ double alpha_eff(double r1){
	double a = Gas_parameters_c[1];
    return a * 2 * log(r1);

}
// 面密度梯度
__device__ double sigma_slope_p(double r1){
	double a = Gas_parameters_c[1];
	return a * 2 * log(r1);
}
// 温度密度梯度
__device__ double temp_slope_q(double r1){
	double b = Gas_parameters_c[2];
	return 1.0 - 2.0 * b;
}
// 熵系数
__device__ double entropy_slope_xi(double r1, double gamma_eff){
	double p = sigma_slope_p(r1);
	double q = temp_slope_q(r1);
	return q - (gamma_eff - 1.0) * p;
}
// Lindblad + barotropic corotation + entropy-related corotation
__device__ double compute_type1_torque_unsat(
	double m, double Msun, double r, double Sigma, double H
){
	// fixed gamma_eff, no new parameter slot
	const double gamma_eff = 1.4;
	double p  = sigma_slope_p(r);               // p = -dlnSigma/dlnr
	double q  = temp_slope_q(r);                // q = -dlnT/dlnr
	double xi = entropy_slope_xi(r, gamma_eff); // entropy slope
	if(H <= 0.0 || r <= 0.0 || Sigma <= 0.0 || Msun <= 0.0 || m <= 0.0){
		return 0.0;
	}
	double h_aspect = H / r;
	double Omega = sqrt(def_ksq * Msun / (r * r * r));
	double qplanet = m / Msun;
	double Gamma0 = (qplanet / h_aspect) * (qplanet / h_aspect) * Sigma * r * r * r * r * Omega * Omega;
	double Gamma_L      = (-2.5 - 1.7 * q + 0.1 * p) / gamma_eff * Gamma0;
	double Gamma_C_baro = (1.1 * (1.5 - p)) / gamma_eff * Gamma0;
	double Gamma_C_ent  = (7.9 * xi) / (gamma_eff * gamma_eff) * Gamma0;
	return Gamma_L + Gamma_C_baro + Gamma_C_ent;
}
// *************************************************
...
//	}
	...
	if(ig < G_Nr_g){
		...
		double Sigma = G_Sigma10 * pow(rg, -alpha_r(rg));
...
__global__ void gasTable_kernel(double *Gas_rg_d, double *Gas_zg_d, double *Gas_rho_d, double3 *GasDisk_d, double3 *GasAcc_d, const int G_Nr_g, const int G_Nr_p){
	...
	if(ip < G_Nr_p){	
		...
		double Sigma = G_Sigma10 * pow(rp, -alpha_r(rp));
//		if(uniform != 1){
...
//r1 = id * 0.01;
		...
		if(r1 > rg0 && r1 < rg1 && x4.z/r1 < 1.5){ //otherwise there is no gas
			...
			double Sigma = G_Sigma10 * pow(r1, -alpha_r(r1));
...
//printf("%d %g %g %g %g %g | %g\n", id, r1, Sigma, h, a_r, a_z, erf(zh));
			...
			if(Sigma > 0.0){
				double G_alpha_eff   = alpha_eff(r1);   // ← 提前计算，后续复用
				//Enhanced Drag
				double Soft;
				...
				if(UsegasDrag == 1 || (UsegasDrag == 2 && m < Mgiant)){
					...
					double eta = 0.5 * ((G_alpha_eff + 1.75) * h * h + 0.5 * x4.z * x4.z) / (r1 * r1);
				...
				if(UsegasTidalDamping == 1 || (UsegasTidalDamping == 2 && m < Mgiant)){
					...
					// keep old wave timescale only for e/i damping
					...
					if(m < Mgiant){
						double Gamma_tot = compute_type1_torque_unsat(m, Msun, r1, Sigma, h);
						a_th += Gamma_tot / (m * r1);
					}

					//keep old e/i damping
